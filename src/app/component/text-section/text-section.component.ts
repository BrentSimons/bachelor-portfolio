...
export class TextSectionComponent implements OnChanges {
  ...
  ngOnChanges() {
    if (this.textSection) {
      ...
      this.firstDescriptions = this.textSection.description.slice(0, this.textSection.description.length / 2);
      this.lastDescriptions = this.textSection.description.slice(this.textSection.description.length / 2);
    }
  }
